package com.uermanagement.controller;
...
public class MyServlet extends HttpServlet {
	...
	private void selectUserById(HttpServletRequest request, HttpServletResponse response) {
		 int id = Integer.parseInt(request.getParameter("id"));
		 UserBean s =UserDao.selectUser(id);
	...
	private void insertUser(HttpServletRequest request, HttpServletResponse response) {
		...
		String email =request.getParameter("email");
		String city =request.getParameter("city");
		UserBean s = new UserBean(name,email,city);
		UserDao.insertUserDetails(s);
		try {
			response.sendRedirect("list");
		} catch (IOException e) {
			// TODO Auto-generated catch block
	...
	private void displayUser(HttpServletRequest request, HttpServletResponse response) {
		  ArrayList<UserBean> al =UserDao.display();
		  request.setAttribute("namelist", al);
